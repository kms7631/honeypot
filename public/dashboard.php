...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['ban_ip'])) {
        ...
        $stmt->execute([$ip]);
        $stmt = $pdo->prepare('UPDATE attack_log SET is_banned = 1 WHERE ip_address = ?');
        $stmt->execute([$ip]);
        header('Location: dashboard.php'); exit;
    }
    if (isset($_POST['unban_ip'])) {
        ...
        $stmt->execute([$ip]);
        $stmt = $pdo->prepare('UPDATE attack_log SET is_banned = 0 WHERE ip_address = ?');
        $stmt->execute([$ip]);
        header('Location: dashboard.php'); exit;
    }
}
    ...
    <table>
        ...
        <tr>
            <td style="text-align:left;">
                ...
                <?php if ($admin_ip && $ban_ip === $admin_ip): ?>
                    <span class="danger" style="margin-left:8px; font-size:0.95em;">(현재 접속 IP)</span>
                <?php endif; ?>
            </td>
            ...
            <td>
                ...
                <?php if ($admin_ip && $ban_ip === $admin_ip): ?>
                    <div style="margin-top:6px; color:#888; font-size:0.92em;">복구용: 해제만 가능합니다.</div>
                <?php endif; ?>
            </td>
        </tr>
