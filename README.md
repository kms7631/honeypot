...
2. `app/config.sample.php`를 `app/config.php`로 복사한 뒤, DB 정보(DB_HOST, DB_NAME, DB_USER, DB_PASS)와 관리자 계정(ADMIN_ID, ADMIN_PW) 설정
...
- 정적 파일(CSS/JS)은 `public/assets/` 기준으로 로드됩니다. (DocumentRoot=public 기준)
- 로그아웃은 `/logout.php`로 동작합니다.
- 관리자 IP를 실수로 차단해도 복구할 수 있도록, 차단 체크에서 대시보드 로그인 페이지는 예외 처리되어 있습니다.

## 설정 팁(가볍게 쓰기)

- 비밀번호 원문 저장은 기본 비활성화입니다. 필요하면 `app/config.php`의 `HONEYPOT_STORE_RAW_PW`를 `true`로 바꾸면 대시보드에서 "원문 보기"가 활성화됩니다.
## 테스트 시나리오
